...
///
/// Conforming types provide an associated ``Request`` type and an instance of
/// it. Protocol extensions handle JSON encoding, set `Content-Type` and
/// `Accept` headers, and default the HTTP method to `POST`.
...
public extension JSONRequestEndpoint {
  /// The HTTP header fields for JSON requests.
  ///
  ...
  /// The HTTP method for JSON requests.
  ///
  /// Defaults to `"POST"`.
